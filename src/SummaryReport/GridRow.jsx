...
export class GridRow extends Component {
    ...
    render() {
        ...
        const {children, row, context, character, summary} = this.props;
            ...
            <tr className={classType + " character " + (row % 2 === 0 ? "even" : "odd") + (!isLoaded ? " loading": "") + (error ? " error" : "")}>
                {Children.map(children, child => {
                    ...
                    if (isValidElement(child)) {
                        return cloneElement(child, { data: character, summary: summary, context: context, render: (x) => x.renderCell(), renderType: "cell" });
                    }
                    return child;
                })}
            </tr>
